...
if __name__ == "__main__":
    ticker = input("Ingrese el ticker de la acción (ej: AAPL, TSLA, MSFT): ").upper()
    periodo = input("Ingrese el periodo de análisis (1y, 2y, 5y, max): ") or "5y"
    
    print(f"\n🔍 Analizando {ticker} para un periodo de {periodo}...")
    sentimiento = analizar_noticias(ticker)
    datos = descargar_datos(ticker, periodo=periodo)
    
    if datos is not None:
        resultado = analizar_accion(datos, sentimiento_global=sentimiento)
        
        # Preguntar si desea guardar los resultados
        guardar = input("\n¿Desea guardar los resultados en archivos CSV? (s/n): ").lower()
        if guardar == 's' or guardar == 'si':
            guardar_resultados(datos, ticker, trades)
    else:
        print(f"❌ No se pudo completar el análisis para {ticker}. Verifique el ticker e intente nuevamente.")
